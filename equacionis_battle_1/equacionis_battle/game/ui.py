...
def draw_hp_box(surface, x, y, nome, level, hp, hp_max, mostrar_exp=False):
    ...
    txt_nome = _font(int(bh * 0.14)).render(nome, True, C.BORDER_DARK)
    surface.blit(txt_nome, (x + int(bw * 0.08), y + int(bh * 0.10)))
    txt_lv = _font(int(bh * 0.11)).render(f"Lv{level}", True, C.BORDER_DARK)
    ...
    if mostrar_exp:
        hp_int = int(hp)
        txt_hp = _font(int(bh * 0.09)).render(f"{hp_int}/{hp_max}", True, C.BORDER_DARK)
