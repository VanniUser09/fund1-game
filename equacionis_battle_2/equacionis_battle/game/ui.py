...
def draw_hp_box(surface, x, y, nome, level, hp, hp_max, mostrar_exp=False):
    ...
    # ---- texto: nome a esquerda, level a direita ----
    # Tamanhos FIXOS calibrados para a fonte Pokemon Classic (que eh mais larga que Arial)
    if mostrar_exp:   # player
        fonte_nome = _font(17)
        fonte_lv   = _font(13)
        fonte_hp   = _font(12)
    else:             # boss (caixa menor)
        fonte_nome = _font(13)
        fonte_lv   = _font(11)
        fonte_hp   = _font(10)

    txt_nome = fonte_nome.render(nome, True, C.BORDER_DARK)
    txt_lv   = fonte_lv.render(f"Lv{level}", True, C.BORDER_DARK)

    y_texto = y + int(bh * 0.14)
    surface.blit(txt_nome, (x + int(bw * 0.08), y_texto))
    surface.blit(txt_lv,
                 (x + bw - txt_lv.get_width() - int(bw * 0.08), y_texto + 3))

    # HP numerico abaixo da barra (somente no HUD do jogador)
    if mostrar_exp:
        hp_int = int(hp)
        txt_hp = fonte_hp.render(f"{hp_int}/{hp_max}", True, C.BORDER_DARK)
        surface.blit(txt_hp,
                     (x + bw - txt_hp.get_width() - int(bw * 0.08),
                      y + bar_y2 + 6))
...
def draw_dialog_box(surface, texto_visivel: str, mostrar_seta: bool = False):
    ...
    for i, linha in enumerate(linhas[:3]):
        ...
        surface.blit(txt, (dx + 24, dy + 24 + i * 26))
...
def draw_input_box(surface, texto_digitado: str, equacao: str):
    ...
    dica = fonte_dica.render("ENTER para confirmar - BACKSPACE para apagar",
                             True, (200, 200, 210))
    surface.blit(dica, (ix + 28, iy + INPUT_SIZE[1] - 24))
...
def draw_end_screen(surface, vitoria: bool):
    ...
    txt_b = _font(14).render("Jogar novamente [R]", True, C.BORDER_DARK)
    surface.blit(txt_b, (btn.centerx - txt_b.get_width() // 2,
                         btn.centery - txt_b.get_height() // 2))

    dica = fonte_s.render("R para reiniciar  -  ESC para sair",
                          True, (200, 200, 210))
